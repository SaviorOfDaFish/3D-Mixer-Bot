...
function renderActivePet() {
  ...
  const artPath = pet.key === "veilkin" ? "/assets/pets/veilkin.png" : null;
  if (artPath) {
...
async function boot() {
  try {
    ...
    document.getElementById("status").textContent="✅ Phase G.5 loaded. Two-panel Home, Veilkin art, leaderboards, and expanded starter customization are active.";
  } catch (err) {
    console.error(err);
