...
function applyAppearanceToAvatar(avatar, appearance) {
  ...
  avatar.dataset.weapon = appearance.weapon || "bow";

  avatar.querySelectorAll(".skin-fill").forEach(el => el.setAttribute("fill", appearance.skin));
  avatar.querySelectorAll(".hair-layer path, .beard-layer path").forEach(el => el.setAttribute("fill", appearance.hairColor));
  avatar.querySelectorAll(".hair-stroke").forEach(el => el.setAttribute("stroke", appearance.hairColor));
  const eyeLeft = avatar.querySelector("#eyeLeft");
  const eyeRight = avatar.querySelector("#eyeRight");
  if (eyeLeft) eyeLeft.setAttribute("fill", appearance.eyeColor || "#4a6d84");
  if (eyeRight) eyeRight.setAttribute("fill", appearance.eyeColor || "#4a6d84");

  const face = avatar.querySelector("#faceShape");
  if (face) {
    if (appearance.eyes === "narrow") face.dataset.eyeStyle = "narrow";
    else if (appearance.eyes === "wide") face.dataset.eyeStyle = "wide";
    else face.dataset.eyeStyle = "round";
  }
}
function renderMainAvatar() {
  applyAppearanceToAvatar(document.getElementById("avatarPreview"), savedAppearance);
}
...
function renderActivePet() {
  ...
  const petFallback = document.getElementById("petIcon");
  if (pet.key === "rime_sprite") {
    petImage.src = "/assets/pets/rimebit.svg";
    petImage.alt = `${petDisplayName(pet)} companion art`;
    petImage.classList.remove("hidden");
    petFallback.classList.add("hidden");
  } else {
    petImage.classList.add("hidden");
    petFallback.classList.remove("hidden");
    petFallback.textContent = pet.icon;
  }
...
function cloneAvatarForEditor() {
  ...
  applyAppearanceToAvatar(clone,workingAppearance);
}
function updateEditor() {
  applyAppearanceToAvatar(document.querySelector("#editorAvatarHost .hunter-art-wrap"),workingAppearance);
  syncSelectedStates();
}
...
async function boot() {
  try {
    ...
    document.getElementById("status").textContent="✅ Phase D.5 loaded. The Hunter and Rimebit now use the final layered art system; live save data remains untouched.";
  } catch (err) {
    console.error(err);
