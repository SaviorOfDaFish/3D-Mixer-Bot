...
function setMonsterStageBackground(stageId, encounter, zoom="normal") {
  ...
  if (encounter?.image) {
    const scale = zoom === "zoomed" ? "125% auto" : zoom === "result" ? "cover" : "contain";
    const position = zoom === "zoomed" ? "center 38%" : zoom === "result" ? "center 28%" : "center center";

    stage.style.backgroundImage =
      `linear-gradient(90deg, rgba(2,6,23,.70), rgba(2,6,23,.15) 30%, rgba(2,6,23,.10) 70%, rgba(2,6,23,.72)), ` +
      `linear-gradient(180deg, rgba(2,6,23,.35), rgba(2,6,23,.05) 35%, rgba(2,6,23,.55)), ` +
      `url("${encounter.image}")`;
    stage.style.backgroundSize = `cover, cover, ${scale}`;
    stage.style.backgroundPosition = `center, center, ${position}`;
    stage.style.backgroundRepeat = "no-repeat";
  } else {
    stage.style.backgroundImage =
      `linear-gradient(rgba(5,10,18,.10),rgba(5,10,18,.18)), url("/assets/backgrounds/hunt-ground-bg.svg")`;
...
async function boot() {
  try {
    ...
    document.getElementById("status").textContent="✅ Phase G.8.1 loaded. Monster artwork now becomes the Encounter, Attempt, and Result backgrounds.";
  } catch (err) {
    console.error(err);
