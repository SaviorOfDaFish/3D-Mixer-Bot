...
let h1Auth = null;
async function ensureH1Auth() {
  if (h1Auth) return h1Auth;
  h1Auth = await window.discordActivityReady;
  return h1Auth;
}
function activityProxyUrl(url) {
  const path = String(url || "");
  if (path.startsWith("/.proxy/")) return path;
  if (path.startsWith("/")) return `/.proxy${path}`;
  return path;
}
async function activityFetch(url, options={}) {
  const auth = await ensureH1Auth();
  const headers = new Headers(options.headers || {});
  headers.set("Authorization", `Bearer ${auth.accessToken}`);
  return fetch(activityProxyUrl(url), {...options, headers, cache:options.cache || "no-store"});
}


let h0VolumeStatus = null;

async function loadH0VolumeStatus() {
  try {
    const response = await fetch("/.proxy/api/h0/status", { cache:"no-store" });
    if (!response.ok) return;
    h0VolumeStatus = await response.json();

    const status = document.getElementById("status");
    if (status) {
      status.textContent =
        `💾 H.0.1 Volume: ${h0VolumeStatus.volumeDetected ? "Mounted" : "Local fallback"} • ` +
        `${h0VolumeStatus.players} saved players • ` +
        `${h0VolumeStatus.botReady ? "Discord bot connected" : "Bot login disabled/not ready"} • ` +
        `Activity writes OFF`;
    }
  } catch (error) {
    console.warn("H.0 status load failed:", error);
  }
}


function applyG9Backgrounds() {
  // Main HOME split character scene
  document.querySelectorAll(".compact-scene").forEach(el => {
    el.style.setProperty("background",
      'linear-gradient(rgba(2,6,23,.10),rgba(2,6,23,.16)), url("/assets/backgrounds/hunt_home.png") center / cover no-repeat',
      "important");
  });

  // Hunt overview player scene
  document.querySelectorAll(".overview-world-card").forEach(el => {
    el.style.setProperty("background",
      'linear-gradient(rgba(2,6,23,.14),rgba(2,6,23,.22)), url("/assets/backgrounds/hunt_battle.png") center / cover no-repeat',
      "important");
  });

  // Zone cards use the hunt battle art rather than the camp.
  document.querySelectorAll(".hunt-zone-art").forEach(el => {
    el.style.setProperty("background",
      'linear-gradient(rgba(2,6,23,.28),rgba(2,6,23,.34)), url("/assets/backgrounds/hunt_battle.png") center / cover no-repeat',
      "important");
  });
}


// ===== PHASE G.8.5 BACKGROUND ROUTING =====
// Home = hunter camp. Hunt overview = dark hunting ground.
// Encounter/Attempt/Result continue to use monster artwork.
function applyPhaseG85Backgrounds() {
  const hunterScene = document.querySelector(".hunter-scene");
  if (hunterScene) {
    hunterScene.style.backgroundImage =
      'linear-gradient(rgba(2,6,23,.10),rgba(2,6,23,.16)),url("/assets/backgrounds/hunt_home.png")';
    hunterScene.style.backgroundSize = "cover";
    hunterScene.style.backgroundPosition = "center";
    hunterScene.style.backgroundRepeat = "no-repeat";
  }

  const huntSceneSelectors = [
    ".hunt-overview-hero",
    ".hunt-overview-scene",
    ".hunting-ground-scene",
    ".hunt-ground-scene"
  ];
  huntSceneSelectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      el.style.backgroundImage =
        'linear-gradient(rgba(2,6,23,.16),rgba(2,6,23,.24)),url("/assets/backgrounds/hunt_battle.png")';
...
// PHASE G.8.2: Hollow King art is embedded directly in this JS bundle.
// This avoids broken GitHub asset paths inside Discord/Railway.
const EMBEDDED_MONSTER_ART = {
  "The Hollow King": "data:image/webp;base64,"
};

function monsterArtSource(encounter) {
  if (!encounter) return null;
  return EMBEDDED_MONSTER_ART[encounter.name] || encounter.image || null;
}
...
let phaseG9Social = null;
let h2aLiveEvents = null;
let incubationTimer = null;
...
function showPetDetail(key) {
  ...
  if (equip) equip.onclick = async () => {
    const response=await activityFetch("/api/activity/pet/equip",{
      method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({petId:p.id})
    });
    const payload=await response.json();
    if(!response.ok) return document.getElementById("status").textContent=`❌ ${payload.error||"Could not equip pet."}`;
    activePetKey=p.key;
    gameData.ownedPets.forEach(x=>x.equipped=String(x.id)===String(p.id));
    renderActivePet(); renderPets(); closeDetail();
    document.getElementById("status").textContent=`✅ ${petDisplayName(p)} is now your active companion.`;
  };
...
async function commitPetName(clear=false) {
  ...
  const response=await activityFetch("/api/activity/pet/name",{
    method:"POST",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({petId:pet.id,nickname:raw})
  });
  const payload=await response.json();
  if(!response.ok) return document.getElementById("status").textContent=`❌ ${payload.error||"Could not rename pet."}`;
  pet.nickname=payload.nickname||null;
  delete localPetNicknames[renameTargetPetKey];
  savePetNicknames();
  closeRenamePet();
  renderActivePet();renderPets();renderDex();
  document.getElementById("status").textContent=payload.nickname
    ? `✅ ${pet.name} is now named ${payload.name}.`
...
let activityResultTimer=null;
function showActivityResult(icon,title,text){
  const popup=document.getElementById("activityResultPopup");
  if(!popup) return;
  document.getElementById("activityResultIcon").textContent=icon||"✨";
  document.getElementById("activityResultTitle").textContent=title||"Success!";
  document.getElementById("activityResultText").textContent=text||"";
  popup.classList.remove("hidden");
  popup.classList.remove("result-pop");
  void popup.offsetWidth;
  popup.classList.add("result-pop");
  clearTimeout(activityResultTimer);
  activityResultTimer=setTimeout(()=>popup.classList.add("hidden"),6500);
}
document.getElementById("activityResultClose")?.addEventListener("click",()=>{
...
async function refreshH2BInventory() {
  try {
    ...
    const status=document.getElementById("status");
    if(status) status.textContent=`❌ Inventory sync failed: ${error.message}`;
  }
}

async function refreshH2BMerchant() {
  const host=document.getElementById("merchantContent");
  try {
    const response=await activityFetch("/api/activity/merchant");
    const payload=await response.json();
    if(!response.ok || !payload.ok) throw new Error(payload.error||"Could not load merchant.");
    const liveTokens=Number(payload.tokens||0);
    document.getElementById("merchantTokenCount").textContent=`🪙 ${liveTokens}`;
    document.getElementById("tokens").textContent=liveTokens;
    if(hunter) hunter.tokens=liveTokens;
    if(!payload.active){
      host.innerHTML=`<div class="empty-state"><b>No merchant is currently visiting.</b><br><small>When a traveling merchant arrives, the live shop will appear here automatically.</small></div>`;
      return;
    }
      ...
      <div class="merchant-live-grid">${(payload.offers||[]).map(o=>`
        <article class="inventory-card merchant-offer ${o.soldOut?"zero":""}">
          ...
          <div class="inventory-body">
            ...
            <p>${o.description||""}</p>
            <p class="card-ability">${o.effect||""}</p>
            <button class="primary-button merchant-buy-btn" data-buy-item="${o.key}" ${o.soldOut?"disabled":""}>${o.soldOut?"SOLD OUT":"Buy"}</button>
          </div>
        </article>`).join("")}</div>`;
    host.querySelectorAll("[data-buy-item]").forEach(btn=>{
      btn.onclick=async()=>{
        btn.disabled=true;
        const original=btn.textContent;
        btn.textContent="Purchasing...";
        try{
          const response=await activityFetch("/api/activity/merchant/buy",{
            method:"POST",headers:{"Content-Type":"application/json"},
            body:JSON.stringify({itemKey:btn.dataset.buyItem})
          });
          ...
          if(result.merchant){
            const purchaseTokens=Number(result.merchant.tokens||0);
            document.getElementById("tokens").textContent=purchaseTokens;
            document.getElementById("merchantTokenCount").textContent=`🪙 ${purchaseTokens}`;
            if(hunter) hunter.tokens=purchaseTokens;
          }
          if(gameData && Array.isArray(result.inventory)) gameData.inventory=result.inventory;
          showActivityResult(
            result.purchased.icon||"🛒",
            `${result.purchased.name} Purchased!`,
            `Added to your inventory. You now have ${Number(result.merchant?.tokens ?? hunter?.tokens ?? 0)} Hunt Tokens.`
          );
          await refreshH2BMerchant();
        }catch(error){
          showActivityResult("❌","Purchase Failed",error.message);
  ...
  grid.querySelectorAll("[data-use-item]").forEach(btn=>{
    btn.onclick=async()=>{
      ...
      try{
        ...
        showActivityResult("❌","Could Not Use Item",error.message);
        btn.disabled=false;btn.textContent=original;
      }
    };
  });
...
function renderRecentHunts() {
  const feed=document.getElementById("recentHuntsFeed");
  if(!feed||!phaseG9Social) return;
  const entries=Array.isArray(phaseG9Social.recentHunts)?phaseG9Social.recentHunts:[];
  feed.innerHTML=entries.length?entries.map(entry=>`
    <div class="recent-hunt-row">
      <span class="recent-hunt-icon">${entry.shiny?"✨":(entry.icon||"🏹")}</span>
      <div class="recent-hunt-copy">
        <b>${entry.player||"Hunter"}</b>
        <span>caught <strong>${entry.monster||"a monster"}</strong></span>
        <small>${entry.rarity||""}${entry.points?` • +${entry.points} pts`:""}${entry.habitat?` • ${entry.habitat}`:""}</small>
      </div>
      <time>${h2aAge(entry.ageMs)}</time>
    </div>`).join("")
    :`<div class="empty-state">No catches yet this season. Be the first!</div>`;
}
...
async function refreshH2AEggData() {
  try {
    const response=await activityFetch("/api/activity/sync");
    const payload=await response.json();
    if(!response.ok || !payload.ok) throw new Error(payload.error||"Could not sync live egg data.");
    phaseG9Social={
      ...(phaseG9Social||{}),
      eggs:Array.isArray(payload.eggs)?payload.eggs:[],
      incubatorSlots:Number(payload.incubators?.slots||1),
      incubations:Array.isArray(payload.incubators?.incubations)?payload.incubators.incubations:[],
      recentHunts:Array.isArray(payload.recentHunts)?payload.recentHunts:[]
    };
    renderEggs();
  } catch(error) {
    console.error("Live egg sync failed:",error);
    const status=document.getElementById("status");
    if(status) status.textContent=`❌ Egg sync failed: ${error.message}`;
  }
}

let h2aEggTimer=null;
function renderEggs() {
  if(!phaseG9Social) return;
  if(h2aEggTimer) clearTimeout(h2aEggTimer);
  h2aEggTimer=setTimeout(()=>{
    if(currentScreen==="eggs") refreshH2AEggData();
  },10000);
  const eggs=Array.isArray(phaseG9Social.eggs)?phaseG9Social.eggs:[];
  const incubations=Array.isArray(phaseG9Social.incubations)?phaseG9Social.incubations:[];
  const slots=Number(phaseG9Social.incubatorSlots||1);
  ...
  grid.innerHTML=eggs.length?eggs.map((egg,index)=>`
    <button class="egg-card" data-egg-index="${egg.inventoryIndex ?? index}">
      ...
      <div class="egg-card-body">
        ...
        <h3>${egg.name}</h3>
        <p>Ready to incubate</p>
        <small>Click to place in an open incubator.</small>
      </div>
    </button>`).join("")
    :`<div class="empty-state">You do not own any eggs yet. Successful hunts can discover eggs.</div>`;

  grid.querySelectorAll("[data-egg-index]").forEach(btn=>{
    btn.onclick=async()=>{
      const response=await activityFetch("/api/activity/egg/incubate",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({inventoryIndex:Number(btn.dataset.eggIndex)})
      });
      const payload=await response.json();
      if(!response.ok) return document.getElementById("status").textContent=`❌ ${payload.error||"Could not incubate egg."}`;
      document.getElementById("status").textContent="🥚 Egg placed in the incubator!";
      await refreshH2AEggData();
    };
  });
  ...
  if(!first){
    document.getElementById("incubatorProgressFill").style.width="0%";
    document.getElementById("incubatorTimeLeft").textContent="—";
    document.getElementById("incubatorPercent").textContent="0%";
    hatchBtn.classList.add("hidden");
  } else {
    const total=Math.max(1,Number(first.readyAt)-Number(first.startedAt));
    const remaining=Math.max(0,Number(first.readyAt)-Date.now());
    const pct=Math.min(100,Math.max(0,((Date.now()-Number(first.startedAt))/total)*100));
    document.getElementById("incubatorProgressFill").style.width=`${pct}%`;
    document.getElementById("incubatorTimeLeft").textContent=remaining<=0?"Ready":formatCountdown(remaining);
    document.getElementById("incubatorPercent").textContent=`${Math.floor(pct)}%`;
  ...
  // Show every active slot below the main incubator card.
  let list=document.getElementById("h2aIncubatorList");
  if(!list){
    list=document.createElement("div");
    list.id="h2aIncubatorList";
    list.className="h2a-incubator-list";
    document.querySelector(".incubator-card")?.insertAdjacentElement("afterend",list);
  }
...
console.log("[Monster Hunt] H10.6.35 WORKING-BASE merge loaded");
boot();
...
function h81ApplyGeneratedHunterToHost(host,imageUrl){
  if(!host) return;
  const legacyAvatar=Array.from(host.children||[]).find(el=>el.classList?.contains("avatar"));
  const legacyShadow=Array.from(host.children||[]).find(el=>el.classList?.contains("avatar-shadow"));
  let img=Array.from(host.children||[]).find(el=>el.classList?.contains("h81-live-hunter-image")) || null;

  if(imageUrl){
    // Generated Hunter art is the ONLY player figure shown. The legacy CSS
    // mannequin remains in the DOM for the creator/fallback system, but is
    // explicitly hidden here so it can never appear behind the real artwork.
    ...
    const huntClone=host.id==="huntOverviewAvatar" || Boolean(host.closest?.("#huntOverviewAvatarHost"));
    if(huntClone){
      host.style.setProperty("width","150px","important");
      host.style.setProperty("height","210px","important");
      img.style.maxWidth="140px";
      img.style.maxHeight="200px";
    }else{
      host.style.setProperty("width","138px","important");
      host.style.setProperty("height","205px","important");
      img.style.maxWidth="130px";
      img.style.maxHeight="195px";
    }
