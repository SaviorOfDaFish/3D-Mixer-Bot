...
async function boot() {
  try {
    const [hunterRes,dataRes,eventRes,huntRes,alignRes] = await Promise.all([
    ...
    if (!hunterRes.ok || !dataRes.ok || !eventRes.ok || !huntRes.ok || !alignRes.ok) throw new Error("Failed to load DEV data.");
    ...
    document.getElementById("status").textContent="✅ Phase G loaded. Hunt gameplay is aligned to the existing Monster Hunt loop; no Approach/Observe mechanics were added.";
  } catch (err) {
    console.error(err);
