...
function searchEncounter() {
  ...
  const monsterVisual = document.getElementById("monsterVisual");
  if (currentEncounter.image) {
    monsterVisual.innerHTML = `<img class="encounter-monster-image" src="${currentEncounter.image}" alt="${currentEncounter.name} art" />`;
  } else {
    monsterVisual.textContent = currentEncounter.icon;
  }
  ...
  if (currentEncounter.level) {
    monsterLevel.textContent = `Level ${currentEncounter.level}`;
    monsterLevel.classList.remove("hidden");
  } else {
    monsterLevel.classList.add("hidden");
  }
...
function showCaptureSuccess(roll, chance, tool, secondChance=false) {
  ...
  const rewardRows = [];
  rewardRows.push(`<span>⭐ Hunter Points: +${pointReward}</span>`);
  if (tokenReward) rewardRows.push(`<span>🪙 Hunt Tokens: +${tokenReward}</span>`);
  rewardRows.push(`<span>🐾 Pet XP: +${petXpReward}</span>`);
  drops.forEach(drop => rewardRows.push(`<span>${drop.icon || "🎒"} ${drop.name}: ×${drop.qty || 1}</span>`));
  extra.forEach(x => rewardRows.push(`<span>${x}</span>`));
  if (secondChance) rewardRows.push(`<span>💎 Companion Effect: Second Chance</span>`);

  document.getElementById("huntResultRewards").innerHTML = rewardRows.join("");
...
function resetEncounterForNextHunt() {
  ...
  document.getElementById("monsterLevel").classList.add("hidden");
...
async function boot() {
  try {
    ...
    document.getElementById("status").textContent="✅ Phase G.7 loaded. Fantasy hunt backgrounds, Hollow King art, pet XP progress, and full catch-result receipts are active.";
  } catch (err) {
    console.error(err);
