...
function renderMainAvatar() {
  ...
  if (window.MonsterHuntFinalArt) window.MonsterHuntFinalArt.refresh(savedAppearance);
}
...
async function boot() {
  try {
    ...
    document.getElementById("status").textContent="✅ Phase D.5.2 loaded. Safe final-art renderer is ready; the proven UI remains intact.";
  } catch (err) {
    console.error(err);
