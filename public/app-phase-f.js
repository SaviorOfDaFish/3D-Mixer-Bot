...
function searchEncounter() {
  ...
  document.getElementById("encounterFlavor").textContent =
    activeHuntZone.key === "bounty"
      ? "A suspicious trail has been found. Investigate the signs and attempt to corner the hidden target."
      : `A ${currentEncounter.rarity.toLowerCase()} creature has appeared. Choose your capture method.`;
  ...
  grid.innerHTML = phaseFHunting.captureTools.map(tool => {
    ...
    return `
      <button class="capture-tool ${selectedTool === tool.key ? "selected" : ""}" data-tool="${tool.key}" ${unavailable ? "disabled" : ""}>
        ...
        <small>${tool.bonus >= 100 ? "Guaranteed DEV capture" : tool.bonus ? `+${tool.bonus}% chance` : "No item used"}</small>
...
function selectCaptureTool(key) {
  ...
  if (!captureBtn) {
    ...
    captureBtn.textContent = "🎯 Attempt Capture";
...
function maybeUseRimebitSecondChance(roll, chance, tool) {
  ...
  if (!rimebitSecondChanceUsed && Math.random() < 0.55) {
    ...
    document.getElementById("huntResultText").textContent =
      `Your capture roll was ${roll} against ${chance}%. The creature started to flee, but Rimebit froze the moment long enough for one more attempt.`;
...
function showCaptureSuccess(roll, chance, tool, secondChance=false) {
  ...
  document.getElementById("huntResultText").textContent =
    `${secondChance ? "Rimebit's second chance worked! " : ""}Roll ${roll} succeeded against a ${chance}% DEV capture chance using ${tool.name}.`;
...
function showCaptureFailure(roll, chance, secondChance=false) {
  ...
  document.getElementById("huntResultText").textContent =
    `${secondChance ? "Even after Rimebit's second chance, " : ""}roll ${roll} missed the ${chance}% DEV capture chance.`;
...
function resetEncounterForNextHunt() {
  ...
  document.getElementById("encounterFlavor").textContent = "Choose Search to find a creature.";
  document.getElementById("captureTools").classList.add("hidden");
  document.getElementById("huntResultPanel").classList.add("hidden");
...
async function boot() {
  try {
    const [hunterRes,dataRes,eventRes,huntRes] = await Promise.all([
    ...
    if (!hunterRes.ok || !dataRes.ok || !eventRes.ok || !huntRes.ok) throw new Error("Failed to load DEV data.");
    ...
    document.getElementById("status").textContent="✅ Phase F loaded. Interactive hunting is available in sandbox mode.";
  } catch (err) {
    console.error(err);
