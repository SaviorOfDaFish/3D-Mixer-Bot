...
async function boot() {
  try {
    const [hunterRes,dataRes,eventRes] = await Promise.all([
      fetch("/api/test-hunter",{cache:"no-store"}),
      fetch("/api/phase-d-data",{cache:"no-store"}),
      fetch("/api/phase-e-events",{cache:"no-store"})
    ]);
    if (!hunterRes.ok || !dataRes.ok || !eventRes.ok) throw new Error("Failed to load DEV data.");
    ...
    document.getElementById("status").textContent="✅ Phase E loaded. Big Game, Bounty, and Distortion screens are running in sandbox mode.";
  } catch (err) {
    console.error(err);
